...
function YouMissedLimb(){
	log.innerHTML=(' You missed enemy attacks now!');      // Create a text node 
}
...
function GoblinImg(){
	...
	document.getElementById('AtkDice').style.display='block';
...
// display123.onload=function(){
// 	displayOnImg()
// }

function MonsterDeath(){
		if(goblin.MonsterHealth<=0)
			{alert('end of tutorial hit the Start Page')
		log.innerHTML='end of tutorial hit the startPage'
...
// ATTACK123()
	...
	function chest(){
		...
		var status = document.getElementById("status");
		 					...
		 					status.innerHTML="you rolled " +d+".";
	if (d>=7) {LimbDamage.style.display='block';
					dissapear()
					YouHitLimb()
